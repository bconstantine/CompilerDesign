...
int canStoreValueInSymbolTable(struct expr exprSpec){
    return (symbolTableStoreValue && !exprSpec.isFunc) || exprSpec.isConst;
}

//whether can store default value in symbol table
int canStoreDefaultValue(){
    return symbolTableStoreValue;
}
    ...
    if(!isDeclaration || identifierNotExistInThisScopeChecker(identifierPos)){
        ...
        if(followUpArguments.isEncounterError || (!isDeclaration && callExprIsError)){
            ...
            if(varType == INTEGER_TYPE){
                resultArgument.paramsOrdering[0] = IPARAM;
                resultArgument.eachParamsHowManyElements[0] = 1; //since this is not an array
            }else if (varType == DOUBLE_TYPE){
                resultArgument.paramsOrdering[0] = DPARAM;
                resultArgument.eachParamsHowManyElements[0] = 1; //since this is not an array
            }else if (varType == STRING_TYPE){
                resultArgument.paramsOrdering[0] = SPARAM;
                resultArgument.eachParamsHowManyElements[0] = 1; //since this is not an array
            }else if(varType == BOOL_TYPE){
                resultArgument.paramsOrdering[0] = BPARAM;
                resultArgument.eachParamsHowManyElements[0] = 1; //since this is not an array
            }else{
            ...
            if(followUpArguments.paramsLength > 0){
                ...
                for(int idx = 0; idx < followUpArguments.paramsLength; idx++){
                    resultArgument.paramsOrdering[idx+1] = followUpArguments.paramsOrdering[idx];
                    resultArgument.eachParamsHowManyElements[idx+1] = followUpArguments.eachParamsHowManyElements[idx];
                }
    ...
    if(!isDeclaration || identifierNotExistInThisScopeChecker(identifierPos)){
        ...
        if(followUpArguments.isEncounterError || (!isDeclaration && callExprIsError)){
            ...
            if(correctStartIndices > correctEndIndices){
                ...
                //set the first entry as the result current variable
                if(varType == INTEGER_TYPE){
                    resultArgument.paramsOrdering[0] = IARRPARAM;
                    resultArgument.eachParamsHowManyElements[0] = correctEndIndices - correctStartIndices + 1; //since this is not an array
                }else if (varType == DOUBLE_TYPE){
                    resultArgument.paramsOrdering[0] = DARRPARAM;
                    resultArgument.eachParamsHowManyElements[0] = correctEndIndices - correctStartIndices + 1; //since this is not an array
                }else if (varType == STRING_TYPE){
                    resultArgument.isEncounterError = 1;
                    resultArgument.paramsLength = 1;
                    ...
                    resultArgument.paramsOrdering[0] = BARRPARAM;
                    resultArgument.eachParamsHowManyElements[0] = correctEndIndices - correctStartIndices + 1; //since this is not an array
                }else{
                    // program should not enter here, if it does something is wrong with your program
                    printf("SelfError %d: Something is wrong with the program, it should not enter here, encounter type: ", linenum);
                ...
                if(resultArgument.isEncounterError == 0 && followUpArguments.paramsLength > 0){
                    // input the trailing parameter
                    for(int idx = 0; idx < followUpArguments.paramsLength; idx++){
                        resultArgument.paramsOrdering[idx+1] = followUpArguments.paramsOrdering[idx];
                        resultArgument.eachParamsHowManyElements[idx+1] = followUpArguments.eachParamsHowManyElements[idx];
...
struct expr relationalStatementChecker(struct expr LHS, char* symbol,struct expr RHS){
    ...
    if(LHS.isError == 0 && RHS.isError == 0){
        if(exprIsArray(LHS.exprType) || exprIsArray(RHS.exprType)){
            ...
            //if one of them is func, result is also func (can't be determined in this project)
            if(LHS.isFunc == 1 || RHS.isFunc == 1){
                resExpr.isFunc = 1;
            }else{
                resExpr.isFunc = 0;
            }
    ...
    if(LHS.isError == 0 && RHS.isError == 0){
        ...
        if(exprIsArray(LHS.exprType) || exprIsArray(RHS.exprType)){
            ...
            if(((LHS.exprType == BEXPR) && (RHS.exprType == BEXPR) && (isOrAnd)) || ((isNot) && (RHS.exprType == BEXPR))) {
                ...
                // if one of them is func, for now left unevaluated
                // for not, just look at rhs
                if(((LHS.isFunc == 1 || RHS.isFunc == 1) && isOrAnd) || ( RHS.isFunc == 1 && isNot)){
                    resExpr.isFunc = 1;
                }else{
                    resExpr.isFunc = 0;
    ...
    if(LHS.isError == 0 && RHS.isError == 0){
        if(exprIsArray(LHS.exprType) || exprIsArray(RHS.exprType)){
            ...
            if((LHSAndRHSInt && isRem) || (LHSAndRHSNumerical && isAddSubMultDiv)) {
                ...
                if(isDiv || isRem){
                    if(RHS.isFunc == 1){
                        // print warning, can't check
                        printf("Warning %d:  In the current state because RHS is a function, can't check for zero division error\n", linenum);
                    }else if(!RHS.isConst && !symbolTableStoreValue){
                ...
                if(identifierNotExistInThisScopeChecker(identifierSpec)){
                    ...
                    if(!isValueless){
                        ...
                        if(typeSpec == INTEGER_TYPE){
                            if(canStoreDefaultValue()) toBeInputted.iVal = 0;
                        }else if(typeSpec == DOUBLE_TYPE){
                            if(canStoreDefaultValue()) toBeInputted.dVal = 0;
                        }else if(typeSpec == STRING_TYPE){
                            if(canStoreDefaultValue()) strcpy(toBeInputted.sVal, "");
                        }else if(typeSpec == BOOL_TYPE){
                            if(canStoreDefaultValue()) toBeInputted.boolVal = 0;
                        }else{
                    ...
                    if(isTypeless){
                        toBeInputted.varConstOrResType = exprSpec.exprType;
                    }else if(isValueless){
...
void initialFunctionProcedureDeclaration(int isFunc, struct IdxPair globalIdentifierSpec, int linenum){
    create(debugMode); //create new scope
    ...
    if(ExprSpec.isError == 0){
        if(exprIsArray(ExprSpec.exprType)){
            ...
            if(ExprSpec.exprType == IEXPR) {
                ...
                if(isPut){
                    // differentiate whether expression is a function or not (which value cant be printed)
                    if(ExprSpec.isFunc == 1){
                        if(putCanPrintExpression) printf("%d ---print result: integer function/array access\n", linenum);
                    }else{
                        // print expression value
                        if(canStoreValueInSymbolTable(ExprSpec)){
                            //print value
                            if(putCanPrintExpression) printf("%d ---print result: %i\n", linenum, ExprSpec.iVal);
                        }else{
                            if(putCanPrintExpression) printf("%d ---print result: int expression\n", linenum);
                        }
                    }
                }else{
                    // a result expression
                ...
                if(isPut){
                    // differentiate whether expression is a function or not (which value cant be printed)
                    if(ExprSpec.isFunc == 1){
                        if(putCanPrintExpression) printf("%d ---print result: double function/array access\n", linenum);
                    }else{
                        // print expression value
                        if(canStoreValueInSymbolTable(ExprSpec)){
                            //print value
                            if(putCanPrintExpression) printf("%d ---print result: %lf\n", linenum, ExprSpec.dVal);
                        }else{
                            if(putCanPrintExpression) printf("%d ---print result: real expression\n", linenum);
                        }
                    }
                }else{
                    // a result expression
                ...
                if(isPut){
                    // differentiate whether expression is a function or not (which value cant be printed)
                    if(ExprSpec.isFunc == 1){
                        if(putCanPrintExpression) printf("%d ---print result: String function/array access\n", linenum);
                    }else{
                        // print expression value
                        if(canStoreValueInSymbolTable(ExprSpec)){
                            //print value
                            if(putCanPrintExpression) printf("%d ---print result: %s\n", linenum, ExprSpec.sVal);
                        }else{
                            if(putCanPrintExpression) printf("%d ---print result: string expression\n", linenum);
                        }
                    }
                }else{
                    // a result expression
                ...
                if(isPut){
                    // differentiate whether expression is a function or not (which value cant be printed)
                    if(ExprSpec.isFunc == 1){
                        if(putCanPrintExpression) printf("%d ---print result: boolean function/array access\n", linenum);
                    }else{
                        // print expression value
                        if(canStoreValueInSymbolTable(ExprSpec)){
                            //print value
                            if(ExprSpec.boolVal){
                                if(putCanPrintExpression) printf("%d ---print result: true\n", linenum);
                            }else{
                                if(putCanPrintExpression) printf("%d ---print result: false\n", linenum);
                            }
                        }else{
                            if(putCanPrintExpression) printf("%d ---print result: bool expression\n", linenum);
                        }
                    }
                }else{
                    // a result expression
                ...
                typePrintingHelperInC(ExprSpec.exprType);
                printf("\n");
            }
        }
    }else{
                ...
                if(isGet || assignmentValid){
                    ...
                    if(varAttributes.varConstOrResType == INTEGER_TYPE){
                        if(isGet){
                            ...
                            if(exprSpec.isFunc || !symbolTableStoreValue){
                                // cannot store value
                                if(!symbolTableStoreValue){
                                    printf("Warning %d: %s cant assign value to statement due to symbol table cant store value\n", linenum, identifierSpec.varName);
                                }else{
                                    printf("Warning %d: %s variable for now will have value unchanged due to its expr is func\n", linenum, identifierSpec.varName);
                                }
                            }else{
...
ProcedureDeclarationConstruct: PROCEDURE IDENTIFIER 
                               {
                                    ...
                                    inAProcedure += 1;
                                }FunctionArgList ProcedureAllStatements END IDENTIFIER
                                {
                                    ...
                                    int endingNameIsSame = strcmp($2.varName, $7.varName) == 0; // back in function/procedure when meeting END, IDENTIFIER name that follows should match
...
FunctionDeclarationConstruct:   FUNCTION IDENTIFIER {
                                    ...
                                    functionStartLine = linenum;
                                    inAFunction += 1;
                                }FunctionArgList COLON Type FunctionAllStatements END IDENTIFIER
                                {
                                    ...
                                    int linenumber = functionStartLine; //declared line number
                                    struct ArgAccumulation argumentList = $4; // List of the argument specification for the function
                                    int endingNameIsSame = strcmp($2.varName, $9.varName) == 0; // back in function/procedure when meeting END, IDENTIFIER name that follows should match
...
FunctionArg:            /*empty*/
                        ...
                        | IDENTIFIER COLON Type FunctionArgFollowUp
                        {
                            ...
                            struct IdxPair identifierPos = $1; //identifierPos: Identifier specification for declaration
                            enum Type varType = $3; //varType: type of identifier that is going to use for declaration
                            struct ArgAccumulation followUpArguments = $4;//followUpArguments, the list of arguments that come after it (because argument is in left recursion)
                        ...
                        | IDENTIFIER COLON ARRAY INTCONST DOT DOT INTCONST OF Type FunctionArgFollowUp
                        {
                            // has at least one function argument, array
                            Trace("Reducing to FunctionArg 3 (Array arg with FunctionArgFollowUp)\n");
                            int isDeclaration = 1; //isDeclaration: 1 if this is a declaration, 0 if it is part of a function call
                            struct IdxPair identifierPos = $1; //identifierPos: Identifier specification for declaration
                            enum Type varType = $9; //varType: type of identifier that is going to use for declaration
                            int startIndices = $4; //startIndices: start index of the array
                            int endIndices = $7; //endIndices: end index of the array
                            struct ArgAccumulation followUpArguments = $10; //followUpArguments, the list of arguments that come after it (because argument is in left recursion)
                            int linenumber = linenum; //current line number
                            int callExprIsError = 0; //this one not used if it is a declaration
...
FunctionArgFollowUp:    COMMA IDENTIFIER COLON Type FunctionArgFollowUp
                        {
                            // has at least one function argument
                            ...
                            struct ArgAccumulation followUpArguments = $5;//followUpArguments, the list of arguments that come after it (because argument is in left recursion)
                            int linenumber = linenum;
                            int callExprIsError = 0; //this one is not used if function is declaration
                            struct ArgAccumulation resultArgument = functionVarArgumentChecker(isDeclaration, identifierPos, varType, followUpArguments, linenumber, callExprIsError);
                            $$ = resultArgument;
                        }
                        | COMMA IDENTIFIER COLON ARRAY INTCONST DOT DOT INTCONST OF Type FunctionArgFollowUp
                        {
                            ...
                            struct ArgAccumulation followUpArguments = $11; //followUpArguments, the list of arguments that come after it (because argument is in left recursion)
                            int linenumber = linenum;
...
ProcedureScope:     NoFuncProcDeclSections ProcNonDeclSections
                    {
                        Trace("Reducing to ProcedureScope 1(NoFuncProcDeclSections and ProcNonDeclSections)\n");
                    }
                    | NoFuncProcDeclSections{
                        Trace("Reducing to ProcedureScope 2 (NoFuncProcDeclSections)\n");
                    }
                    | ProcNonDeclSections{
                        Trace("Reducing to ProcedureScope 3 (ProcNonDeclSections)\n");
                    }
...
FunctionScope:  NoFuncProcDeclSections FuncNonDeclSections
                {
                    Trace("Reducing to ProcedureScope 1(NoFuncProcDeclSections and FuncNonDeclSections)\n");
                }
                | NoFuncProcDeclSections
                {
                    Trace("Reducing to ProcedureScope 2 (NoFuncProcDeclSections)\n");
                ...
                | FuncNonDeclSections
                {
                    Trace("Reducing to ProcedureScope 3 (FuncNonDeclSections)\n");
                }

ProcNonDeclSections:    ProcNonDeclSections ProcNonDeclStatement
                        {
                            Trace("Reducing to ProcNonDeclSections 1 (from ProcNonDeclSections branching to ProcNonDeclStatement)\n");
                        }
                        | ProcNonDeclStatement
                        {
                            Trace("Reducing to ProcNonDeclSections 2 (ProcNonDeclStatement)\n");
                        }

ProcNonDeclStatement:   PutStatement
                        {
                            Trace("Reducing to ProcNonDeclStatement 1 (Put)\n");
                        }
                        | GetStatement
                        {
                            Trace("Reducing to ProcNonDeclStatement 2 (Get)\n");
                        }
                        | AssignmentStatement
                        {
                            Trace("Reducing to ProcNonDeclStatement 3 (Assignment)\n");
                        }
                        | LoopConstruct
                        {
                            Trace("Reducing to ProcNonDeclStatement 4 (Loop)\n");
                        }
                        | BlockConstruct
                        {
                            Trace("Reducing to ProcNonDeclStatement 5 (Block)\n");
                        }
                        | IfConstruct
                        {
                            Trace("Reducing to ProcNonDeclStatement 6 (If)\n");
                        }
                        | ForConstruct
                        {
                            Trace("Reducing to ProcNonDeclStatement 7 (For)\n");
                        }
                        | ReturnStatement
                        {
                            Trace("Reducing to ProcNonDeclStatement 8 (Return)\n");
                        }
                        | Expr 
                        {
                            Trace("Reducing to ProcNonDeclStatement 9 (Expr)\n");
                        }
                        | ResultStatement
                        {
                            ...
                            Trace("Reducing to ProcNonDeclStatement 10 (ResultStatement)\n");
                        }
                        | ExitStatement
                        {
                            //because procedure declaration inside loop/for is invalid
                            printf("Error %d: Invalid! Result statement encountered inside a procedure not a loop\n",linenum);
                            Trace("Reducing to ProcNonDeclStatement 11 (ExitStatement)\n");
                        }
                        | SkipStatement
                        {
                            Trace("Reducing to ProcNonDeclStatement 12 (SkipStatement)\n");
                        }

FuncNonDeclSections: FuncNonDeclSections FuncNonDeclStatement
                    {
                        Trace("Reducing to FuncNonDeclSections 1 (From FuncNonDeclSections branching to FuncNonDeclStatement))\n");
                    }
                    | FuncNonDeclStatement
                    {
                        Trace("Reducing to FuncNonDeclStatement 2 (From FuncNonDeclStatement)\n");
                    }



FuncNonDeclStatement:   PutStatement
                        {
                            Trace("Reducing to FuncNonDeclStatement 1 (Put)\n");
                        }
                        | GetStatement
                        {
                            Trace("Reducing to FuncNonDeclStatement 2 (Get)\n");
                        }
                        | AssignmentStatement
                        {
                            Trace("Reducing to FuncNonDeclStatement 3 (Assignment)\n");
                        }
                        | LoopConstruct
                        {
                            Trace("Reducing to FuncNonDeclStatement 4 (Loop)\n");
                        }
                        | BlockConstruct
                        {
                            Trace("Reducing to FuncNonDeclStatement 5 (Block)\n");
                        }
                        | IfConstruct
                        {
                            Trace("Reducing to FuncNonDeclStatement 6 (If)\n");
                        }
                        | ForConstruct
                        {
                            Trace("Reducing to FuncNonDeclStatement 7 (For)\n");
                        }
                        | ResultStatement
                        {
                            Trace("Reducing to FuncNonDeclStatement 8 (Result)\n");
                        }
                        | Expr 
                        {
                            Trace("Reducing to FuncNonDeclStatement 9 (Expr)\n");
                        }
                        | ReturnStatement
                        {
                            ...
                            Trace("Reducing to FuncNonDeclStatement 10 (ReturnStatement)\n");
                        }
                        | ExitStatement
                        {
                            //because function declaration inside loop/for is invalid
                            printf("Error %d: Invalid! Result statement encountered inside a function not a loop\n",linenum);
                            Trace("Reducing to FuncNonDeclStatement 11 (ExitStatement)\n");
                        }
                        | SkipStatement
                        {
                            Trace("Reducing to FuncNonDeclStatement 12 (SkipStatement)\n");
                        }

NoFuncProcDeclSections: NoFuncProcDeclSections NoFuncProcDeclStatement
                        {
                            Trace("Reducing to NoFuncProcDeclSections 1 (From NoFuncProcDeclSection Branching to NoFuncProcDeclStatement)\n");
                        }
                        | NoFuncProcDeclStatement
                        {
                            Trace("Reducing to NoFuncProcDeclSections 2 (From NoFuncProcDeclStatement)\n");
                        }
...
NonDeclarationStatement: PutStatement
                        ...
                        | Expr 
                        {
                            Trace("Reducing to NonDeclarationStatement 8 (Expr)\n");
                        }
                        | ResultStatement
                        {
                        ...
                        | ExitStatement
                        {
                            ...
                            if(inALoopOrFor <= 0){
                                printf("Error %d: Invalid! Result statement encountered in a scope where it is not directly inside a loop\n",linenum);
                            }
...
ElseOrNot:      /*empty*/
                ...
                {
                    Trace("Reducing to ElseOrNot 2 (Else only)\n");
                    popBack(debugMode);
                }
                | ELSE IfConstruct{
                    //go to another branch of If
                    Trace("Reducing to ElseOrNot 3 (Else If)\n");
                }
...
/*If has no Procedure and Function declaration in it, the other is just like usual*/
IfScope:    NoFuncProcDeclSections NonDeclarationSections
            {
                Trace("Reducing to IfScope (NoFuncProcDeclSections and NonDeclarationSections)\n");
            }
            | NoFuncProcDeclSections{
                Trace("Reducing to IfScope (NoFuncProcDeclSections)\n");
            }
            | NonDeclarationSections{
                Trace("Reducing to NonDeclarationSections\n");
            }
...
LocalScope:     DeclarationSections NonDeclarationSections
                {
                    Trace("Reducing to LocalScope 1 (DeclarationSections and NonDeclarationSections)\n");
                }
                | DeclarationSections
                {
                    Trace("Reducing to LocalScope 2 (DeclarationSections only)\n");
                }
                | NonDeclarationSections
                {
                    Trace("Reducing to LocalScope 3 (NonDeclarationSections only)\n");
                }
...
LoopScope:  NoFuncProcDeclSections LoopNonDeclarationSections
            {
                Trace("Reducing to LoopScope 1 (NoFuncProcDeclSections and LoopNonDeclarationSections)\n");
            }
            | NoFuncProcDeclSections
            {
                Trace("Reducing to LoopScope 2 (NoFuncProcDeclSections)\n");
            }
            | LoopNonDeclarationSections
            {
                Trace("Reducing to LoopScope 3 (LoopNonDeclarationSections)\n");
            }

LoopNonDeclarationSections: LoopNonDeclarationSections LoopNonDeclarationStatement
                            {
                                Trace("Reducing to LoopNonDeclarationSections 1 (Branching of LoopNonDeclarationSections to LoopNonDeclarationStatement)\n");
                            }
                            | LoopNonDeclarationStatement
                            {
                                Trace("Reducing to LoopNonDeclarationSections 2 (From LoopNonDeclarationStatement)\n");
                            }

/*From the NonDeclarationStatement, need to add skip statement, and exit statement*/
LoopNonDeclarationStatement: PutStatement
                            {
                                Trace("Reducing to LoopNonDeclarationStatement 1 (Put)\n");
                            }
                            | GetStatement
                            {
                                Trace("Reducing to LoopNonDeclarationStatement 2 (Get)\n");
                            }
                            | AssignmentStatement
                            {
                                Trace("Reducing to LoopNonDeclarationStatement 3 (Assignment)\n");
                            }
                            | LoopConstruct
                            {
                                Trace("Reducing to LoopNonDeclarationStatement 4 (Loop)\n");
                            }
                            | BlockConstruct
                            {
                                Trace("Reducing to LoopNonDeclarationStatement 5 (Block)\n");
                            }
                            | IfConstruct
                            {
                                Trace("Reducing to LoopNonDeclarationStatement 6 (If)\n");
                            }
                            | ForConstruct
                            {
                                Trace("Reducing to LoopNonDeclarationStatement 7 (For)\n");
                            }
                            | Expr
                            {
                                Trace("Reducing to LoopNonDeclarationStatement 8 (Expr)\n");
                            }
                            | SkipStatement
                            {
                                Trace("Reducing to LoopNonDeclarationStatement 9 (Skip)\n");
                            }
                            | ExitStatement
                            {
                                Trace("Reducing to LoopNonDeclarationStatement 10 (Exit)\n");
                            }
                            | ReturnStatement
                            {
                                ...
                                Trace("Reducing to LoopNonDeclarationStatement 11 (ReturnStatement)\n");
                            }
                            | ResultStatement
                            {
                                ...
                                Trace("Reducing to LoopNonDeclarationStatement 12 (ResultStatement)\n");
                            }

/*Two form, whether they are decreasing or not
Here I assume since no increment/decrement distance, only INTCONST is valid*/
ForConstruct: FOR DecreaseOrNot IDENTIFIER COLON Expr DOT DOT Expr 
              {
                ...
                if($5.isError || $8.isError)
                {
                ...
                else if($5.exprType != IEXPR || $8.exprType != IEXPR){
                    ...
                    if(($2 == 1 && $5.iVal >= $8.iVal) || ($2 == 0 && $5.iVal <= $8.iVal))
                    {
                        // valid
...
ExitStatement:  EXIT ExitCondition
                {
                    if(inALoopOrFor <= 0){
                        printf("Error %d: exit isnt in any loop or function, this will cause syntax error\n", linenum);
                    }
                                ...
                                if(varAttributes.entryType == VAR_ENTTYPE){
                                    ...
                                    if(IndexExprSpec.isError == 0 && !exprIsArray(IndexExprSpec.exprType) && IndexExprSpec.exprType == IEXPR){
                                        ...
                                        if(indexValid){
                                            res.isError = 0;
                                            res.isConst = 0;
                                            res.isFunc = 1; //regard it as function because value is not known
...
FunctionParameters: /*empty*/
                    ...
                    | Expr FunctionParametersFollowUp
                    {
                        ...
                        if(exprIsArray($1.exprType)){
                            ...
                            enum Type varType = translatorExprType($1.exprType); //varType: type of identifier that is going to use for declaration / function or procedure call
                            ...
                            int endIndices = $1.arrLength -1; //endIndices: end index of the array
                            struct ArgAccumulation followUpArguments = $2; //followUpArguments, the list of arguments that come after it (because argument is in left recursion)
                            int linenumber = linenum; //current line number
                            int callExprIsError = $1.isError; //whether expression used for parameters is error
                            ...
                            int isDeclaration = 0; //isDeclaration: 1 if this is a declaration, 0 if it is part of a function call
                            struct IdxPair identifierPos = defaultIdxPairValue(); //identifierPos: Identifier specification for declaration, not used if it is a function call
                            enum Type varType = translatorExprType($1.exprType); //varType: type of identifier that is going to use for declaration
                            struct ArgAccumulation followUpArguments = $2;//followUpArguments, the list of arguments that come after it (because argument is in left recursion)
                            int linenumber = linenum; //current line number
                            int callExprIsError = $1.isError; //whether expression used for parameters is error
...
FunctionParametersFollowUp: /*empty*/
                            ...
                            | COMMA Expr FunctionParametersFollowUp
                            {
                                ...
                                if(exprIsArray($2.exprType)){
                                    ...
                                    struct ArgAccumulation followUpArguments = $3; //followUpArguments, the list of arguments that come after it (because argument is in left recursion)
                                    int linenumber = linenum; //current line number
...
ConstDeclConstruct: CONST IDENTIFIER COLON Type ASSIGNMENT Expr
                    {
                        ...
                        struct expr exprSpec = $6; //if not valueless, send the expression specification
                    ...
                    | CONST IDENTIFIER ASSIGNMENT Expr
                    {
                        ...
                        struct expr exprSpec = $4; //if not valueless, send the expression specification
                        int linenumber = linenum; //declared line number
                        VariableDeclarationSteps(isValueless, isTypeless, isConst, identifierSpec, typeSpec, exprSpec, linenumber);
                    }
                    ;
...
VarDeclConstruct:   VAR IDENTIFIER COLON Type ASSIGNMENT Expr
                    {
                        // complete declaration
